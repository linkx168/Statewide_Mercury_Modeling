...
```{r}
# #infestation year is independently tracked for invaded and uninvaded lakes
# wae_data %>% 
#   group_by( year_infested, nearest_infested_year) %>% 
#   summarize(n = n()) %>% print(n = nrow(.))

#fix by merging, and add in a post_inv dummy, which bakes in the 2 yr lag. 
wae_data %>% 
  mutate(year_infested = ifelse(is.na(year_infested), 
                                nearest_infested_year, 
                                year_infested)) %>% 
  mutate(year_inf_lag2 = year_infested +2) %>% 
  mutate( post_inv = yearcoll>year_inf_lag2) %>% 
  # group_by( year_infested) %>% 
  # summarize(n = n()) %>% print(n = nrow(.))
  {wae_data <<- .}

# #here we see multiple pre- and multiple post- years. Also see that some lakes have no pre or no post
# wae_data %>% group_by(waterway, dow, post_inv) %>% 
#   summarize(nyr = length(unique(yearcoll)))

# wae_data %>% 
#   group_by(waterway, post_inv, zm_lake) %>% 
#   summarize(n = n(), .groups = "drop") %>% 
#   group_by(waterway,zm_lake) %>% 
#   summarize(
#     has_T = any(post_inv == TRUE),
#     has_F = any(post_inv == FALSE),
#     category = case_when(
#       has_T & has_F ~ "Both post- and pre-",
#       has_T ~ "Only post-",
#       has_F ~ "Only pre-"
#     )
#   ) %>% 
#   group_by(category, zm_lake) %>% 
#   count(category)


#now for each lake, calculate the mean year of the pre- data and the mean year of the post- data
wae_data %>% 
...
# ggplot(lake_deltas, aes(delta_t, group =zm_lake)) + 
#   geom_density(aes(color = zm_lake))
# 
# ggplot(lake_deltas, aes(delta_t, group =zm_lake)) + 
#   geom_histogram(aes(color = zm_lake), binwidth = 1)+ 
#   geom_density(aes(color = zm_lake))

# Compute max histogram count for scaling
max_count <- max(ggplot_build(ggplot(lake_deltas, aes(x = delta_t)) + 
                                geom_histogram(binwidth = 1))$data[[1]]$count)

ggplot(lake_deltas, aes(x = delta_t, fill = zm_lake)) +
  geom_histogram(aes(y = ..count..), binwidth = 1, alpha = 0.4, position = "identity") +
  geom_density(aes(y = ..density.. * 5*max_count, color = zm_lake), alpha = 0.01, size = 1) +
  scale_y_continuous(
    name = "Histogram Count",
    sec.axis = sec_axis(~ . / max_count, name = "Density")
  ) +
  labs(x = "Delta T", fill = "ZM Lake", color = "ZM Lake", title = "wae") +
  theme_minimal()
...
for (species in species_list) {
  ...
  # Print plot (optional, only if running interactively)
  print(p)
}
...
#Figure 2 - with all 7 species 
```{r}
#define lengths to be used in the marginal effects
species_lengths <- tibble(
  species = c("WAE", "NOP", "BLC", "BLG", "LMB", "WTS", "YEP"),
  standard_length = c(381, 610, 205, 155, 305, 305, 205) # Adjust as needed
)
...
species_summary <- species_lengths %>%
  mutate(summary_data = map2(species, standard_length, function(sp, len) {
    ...
    species_data <- hg.model %>% 
      filter(tolower(spec) == sp_lower) %>% 
      mutate(lgthmm = lgthin * 25.4)
    
    # Calculate species-specific length scaling
    length_stats <- species_data %>%
      summarise(mean_length = mean(lgthmm), sd_length = sd(lgthmm))
    
    specific_length_scaled <- (len - length_stats$mean_length) / length_stats$sd_length

    # Extract marginal effects from the model
    marg_draws <- emmeans(get(paste0("fit_", sp_lower)), ~ zm_lake * zm_sample_lag_2,
                          at = list(scaled_length = specific_length_scaled),
...
wrap_plots(species_plots$plot, ncol = 1)
...
all_diff %>%
  mutate(species = case_when(species == "BLC" ~ "Black Crappie",
                             ...
                             species == "YEP" ~ "Yellow Perch",
                             TRUE ~ species),
         species = factor(species, levels = species_order)) %>%  # Order species by median value
  ggplot() +
  stat_pointinterval(aes(x = species, y = value, color = species ), .width = c(.66, .95)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ylab("Difference in Mercury Concentration\nBefore and After (ppm)") +
