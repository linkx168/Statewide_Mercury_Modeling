...
hg.model <- hg %>%
  ...
  mutate(zm_infested_lag_1 = Year_Infested +1,
         ...
         zm_lake = case_when(is.na(Year_Infested) ~ "n",
                             TRUE ~ "y"),
         zm_sample_lag_0 = case_when(Year_Infested <=YEARCOLL ~ "y",
                               TRUE ~ "n"),
...
# Data Coverage Scoping 
- this does not need to run prior to analysis, it is exploration 
```{r}
...
#what do we have for kornois?
hg.model %>% 
  filter(dow == "73020002") %>% 
  group_by(yearcoll, spec) %>% 
  count()

hg.model %>% 
  filter(dow == "73020002" & spec == "WAE") %>% 
  ggplot() +
  geom_point(aes(lgthin, hgppm, color = as.factor(yearcoll)), size = 3) +
...
1. We filter out fish below 5" and multiply LOD samples by 0.5
2. Filter function - this filter function takes 1.) the species of interest, 2.) the data and 3.) the lag year variable (y/n) and returns a dataset that has lakes with a least one year of data pre and one year of data post. ZM invasion year is determined by the lag year variable pre filtering and the reference invasion lakes are determined by the closest invaded lake within the analysis
...
hg.model <- hg.model %>% 
  ...
  mutate(hgppm = case_when(hgcode == "K" ~ hgppm*0.5,
                           TRUE ~ hgppm))
...
create_species_data <- function(species, data, lag_variable) {
  ...
  #auto change of lag year with number at the end of zm sample lag
  ref_lakes <- ref_lakes %>% 
    mutate(nearest_zm_lake = st_nearest_feature(geometry, zm_lakes$geometry),
           ...
           dynamic_lag = parse_number(str_extract(lag_variable, "\\d+$")),
           !!sym(lag_variable) := case_when(yearcoll >= (nearest_infested_year + dynamic_lag)  ~ "y",
                                       yearcoll <= (nearest_infested_year + dynamic_lag) ~ "n")) %>% 
    distinct(dow, !!sym(lag_variable)) %>% 
    group_by(dow) %>% 
    count() %>% 
    filter(n >1)
...
#taking the year invaded with 2 years of lag to build reference systems
  ...
  if (nrow(ref_fish) == 0 | nrow(zm_fish) == 0) {
    message(paste("Skipping:", species, "Lag:", lag_variable, "- No data available"))
...
# wts_data <- create_species_data(species = "WTS", data = hg.model, lag_variable = "zm_sample_lag_2")

hg.model <- bind_rows(blc_data, blg_data, lmb_data, nop_data, wae_data, yep_data)

#when looking at the difference between full and filtered for length - diff of 24 samples, only 22 when you directly applied it to the full data set without the filtering script because one lake keeps two bluegill recoreds but loosing post-invasion samples. Thus, when running the filter script, that entire lake-species is filtered out 

# hg.model.full %>% 
#   filter(dow == "32001800" & spec == "YEP") %>% 
#   glimpse()
# 
# diff <- anti_join(hg.model.full, hg.model, 
#                   by = c("dow", "spec", "lgthin"))
# 
# removed <- diff %>% 
#   group_by(dow, spec, zm_sample_lag_2) %>% 
#   summarise(n_removed = n())
# 
# hg.model.full %>% 
#   semi_join(removed, by = c("dow", "spec")) %>%  
#   group_by(dow, spec, zm_lake, zm_sample_lag_2) %>% 
#   count() %>% 
#   left_join(removed) %>% 
#   mutate(prop_removed = n_removed/n) %>% 
#   print(n = nrow(.))

# Define species list and lagged variable names (including non-lagged "zm_sample")
...
for (species in species_list) {
  for (lag_var in lag_variables) {
    object_name <- paste0(species, "_", lag_var)  # Construct variable name
    species_data <- create_species_data(species = species, data = hg.model, lag_variable = lag_var)
    
    assign(object_name, species_data, envir = .GlobalEnv)  # Save in global environment
    
    print(paste("Created:", object_name))  # Print progress
  }
}

all_lags_samples <- bind_rows(BLC_zm_sample_lag_0,
                              BLC_zm_sample_lag_1,
                              BLC_zm_sample_lag_2,
                              BLC_zm_sample_lag_3,
                              BLC_zm_sample_lag_4,
...
glimpse(all_lags_samples)

all_lags_samples <- all_lags_samples %>% 
  mutate(lag = case_when(!is.na(zm_sample_lag_0) ~ "0",
                         !is.na(zm_sample_lag_1) ~ "1",
                         !is.na(zm_sample_lag_2) ~ "2",
                         !is.na(zm_sample_lag_3) ~ "3",
                         !is.na(zm_sample_lag_4) ~ "4",
                         !is.na(zm_sample_lag_5) ~ "5"),
         zm_sample = case_when(!is.na(zm_sample_lag_0) ~ zm_sample_lag_0,
                               !is.na(zm_sample_lag_1) ~ zm_sample_lag_1,
                               !is.na(zm_sample_lag_2) ~ zm_sample_lag_2,
                               !is.na(zm_sample_lag_3) ~ zm_sample_lag_3,
                               !is.na(zm_sample_lag_4) ~ zm_sample_lag_4,
                               !is.na(zm_sample_lag_5) ~ zm_sample_lag_5))

zm_baci_clean <- all_lags_samples %>% 
  rename(lake_name = waterway,
         lake_id = dow,
         year = yearcoll,
         species = spec,
         sample_type = anathg,
...
zm_baci_clean %>% 
  filter(species %in% c("WAE", "YEP", "BLC", "NOP") & lag == "2") %>% 
  group_by(species, zm_lake, zm_sample) %>% 
  count()

glimpse(zm_baci_clean)

zm_baci_clean %>% 
  filter(lag == 2) %>% 
  group_by(sample_type, species, zm_lake) %>% 
  count() %>% 
  arrange(species) %>% 
  print(n = nrow(.))
write_csv(zm_baci_clean, "baci_data_29april2025.csv")
```
...
```{r}
#setting priors 
get_prior(log(hgppm) ~ scaled_length + zm_lake*zm_sample_lag_2 + (1|dow) + (1|yearcoll), 
             data = wae_data, 
             family = gaussian(), 
             iter = 20000,
             warmup = 5000)
...
fit_wts <- readRDS("Output/BACI/baci_update_wts.rds")
...
# saved models from drive
```{r}
fit_wae <- readRDS(file.path("E:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Zebra Mussel Baci Analysis", "Output", "baci_update_wae.rds"))
fit_nop <- readRDS(file.path("E:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Zebra Mussel Baci Analysis", "Output", "baci_update_nop.rds"))
fit_blg <- readRDS(file.path("E:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Zebra Mussel Baci Analysis", "Output", "baci_update_blg.rds"))
fit_lmb <- readRDS(file.path("E:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Zebra Mussel Baci Analysis", "Output", "baci_update_lmb.rds"))
fit_yep <- readRDS(file.path("E:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Zebra Mussel Baci Analysis", "Output", "baci_update_yep.rds"))
fit_blc <- readRDS(file.path("E:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Zebra Mussel Baci Analysis", "Output", "baci_update_blc.rds"))
```


2. looping over all species combos of zm_lag for sensitivity analysis 
```{r}
species_list <- c("WAE", "NOP", "BLG", "BLC", "LMB", "YEP", "WTS")

for (species in species_list) {
  # Adjust lag variables for WTS
  if (species == "WTS") {
    lag_variables <- c(paste0("zm_sample_lag_", 0:3))  # Exclude lag_4 and lag_5
  } else {
    lag_variables <- c(paste0("zm_sample_lag_", 0:5))
  }
  ...
  for (lag_var in lag_variables) {
    ...
    if (!exists(data_name)) {
      next  # Skip to the next iteration if data does not exist
    }
...
#saved model runs
```{r}
species <- c("WAE", "NOP", "BLG", "BLC", "LMB", "YEP", "WTS")
...
Loops over emmeans for each model fit combo of species - creates marginal effects plot of every run for comparing
```{r}
...
length_stats <-  nop_data %>% 
  summarise(mean_length = mean(lgthmm),
            sd_length = sd(lgthmm))
specific_length_scaled <- (610 - length_stats$mean_length) / length_stats$sd_length
...
marg_draws %>% 
  mutate(exp_ppm = exp(.value)) %>% 
  select(zm_lake, zm_sample_lag_2, .draw, exp_ppm) %>% 
  group_by(zm_lake, zm_sample_lag_2) %>% 
  median_qi() %>% 
  write_csv("nop_values_ppm_marg_eff.csv")
...
prob_exceed <- fit_nop %>% 
  ...
  mutate(lgthmm =(scaled_length*sd_length)+mean_length) %>% 
  mutate(is_above_threshold = .prediction > log(.22)) %>%
  group_by(zm_lake, zm_sample_lag_2, lgthmm) %>% 
...
#24 inch fish
prob_exceed %>% 
  mutate(distance = abs(lgthmm - 610)) %>% 
  filter(distance == min(distance)) %>%
  select(zm_lake, zm_sample_lag_2, lgthmm, prob, distance) %>% 
  write_csv("nop_prob_exceeding_24_in.csv")
...
pike_length_probs <- ggplot() +
  geom_line(data = prob_exceed, aes(x = lgthmm, y = prob, color = zm_sample_lag_2)) + 
  geom_point(data = vert_line, aes(x =lgthmm, y = prob, color = zm_sample_lag_2)) +
  geom_line(data = vert_line, aes(x = lgthmm, y = prob, group = zm_lake)) +
...
pike_length_probs <- pike_length_probs +
  theme(legend.position = "none",
...
pike_length_ppm <- ggplot() +
  ...
  theme_bw() +
  geom_ribbon(data = pred_lengths, aes(x = lgthmm, y = median, color = zm_sample_lag_2, ymax = smoothed_uci, ymin = smoothed_lci, fill = zm_sample_lag_2), lwd = 0, alpha = .3) + # alpha is transparency
  geom_line(data = pred_lengths, lwd = 1, aes(x = lgthmm, y = median, color = zm_sample_lag_2, lty = zm_sample_lag_2)) + # center line of ribbon
  geom_point(data = nop_data %>% 
               mutate(zm_lake = case_when(zm_lake == "y" ~ "Zebra Mussel",
                             zm_lake == "n" ~ "Reference")), aes(x = lgthmm, y = hgppm, color = zm_sample_lag_2), alpha = .2) +
...
pike_length_ppm <- pike_length_ppm +
  theme(legend.position = "none",
        axis.title.x = element_blank())
pike_length_ppm
...
pred_lengths %>% 
  ...
  group_by(zm_lake, zm_sample_lag_2) %>% 
  filter(distance == min(distance)) %>%
  select(zm_lake, zm_sample_lag_2, lgthmm) %>% 
  pivot_wider(names_from = c(zm_lake, zm_sample_lag_2),
              values_from = lgthmm) %>% 
  mutate(percent_change_reference = ((Reference_y - Reference_n)/Reference_n)*100,
         percent_change_zm = ((`Zebra Mussel_y` - `Zebra Mussel_n`)/`Zebra Mussel_n`)*100,
         diff_of_change = percent_change_zm - percent_change_reference) %>% 
  write_csv("nop_percent_change_crossing_lengths.csv")
...
marg_draws %>% 
  ...
  group_by(zm_lake, zm_sample_lag_2) %>% 
  median_qi() %>% 
  write_csv("wae_values_ppm_marg_eff.csv")
...
prob_exceed <- fit_wae %>% 
  ...
  mutate(lgthmm =(scaled_length*sd_length)+mean_length) %>% 
  mutate(is_above_threshold = .prediction > log(.22)) %>% 
  group_by(zm_lake, zm_sample_lag_2, scaled_length, lgthmm) %>% 
  summarise(prob = mean(is_above_threshold), .groups = 'drop') %>% 
  mutate(zm_lake = case_when(zm_lake == "y" ~ "Zebra Mussel",
                             zm_lake == "n" ~ "Reference"))
...
prob_exceed %>% 
  mutate(mean_length = length.stats$mean.length,
         sd_length = length.stats$sd.length) %>% 
  mutate(lgthmm =(scaled_length*sd_length)+mean_length) %>% 
  mutate(distance = abs(lgthmm - 381)) %>% 
  filter(distance == min(distance)) %>%
  select(zm_lake, zm_sample_lag_2, lgthmm, prob, distance) %>% 
...
walleye_length_probs <- walleye_length_probs +
  ylab("Probability of\nExceeding 0.22 (ppm)") 
walleye_length_probs
...
pred_lengths %>% 
  ...
  mutate(percent_change_reference = ((Reference_y - Reference_n)/Reference_n)*100,
         percent_change_zm = ((`Zebra Mussel_y` - `Zebra Mussel_n`)/`Zebra Mussel_n`)*100,
         diff_of_change = percent_change_zm - percent_change_reference) %>% 
  write_csv("wae_percent_change_crossing_lengths.csv")
...
##matching Blinick et al. 2024
baci_discount <- pred_lengths %>% 
  filter(zm_lake == "Reference") %>% 
  select(-zm_lake) %>% 
  pivot_wider(
    names_from = zm_sample_lag_2, 
    values_from = c(median, lci, uci, smoothed_median, smoothed_lci, smoothed_uci)
  ) %>% 
  mutate(
    discount_median = median_y - median_n,
    discount_lci = lci_y - lci_n,
    discount_uci = uci_y - uci_n,
    discount_smoothed_median = smoothed_median_y - smoothed_median_n,
    discount_smoothed_lci = smoothed_lci_y - smoothed_lci_n,
    discount_smoothed_uci = smoothed_uci_y - smoothed_uci_n
  ) %>% 
  select(lgthin,
    discount_median, 
         discount_lci,
         discount_uci,
         discount_smoothed_median,
         discount_smoothed_lci,
         discount_smoothed_uci)

pred_lengths_discount <- pred_lengths %>% 
  filter(lgthin > 8.5 & lgthin < 21.6) %>% 
  left_join(baci_discount) %>% 
    mutate(
    # Apply discount only where conditions are met
    median = case_when(
      zm_lake == "Zebra Mussel" & zm_sample_lag_2 == "y" ~ (median - discount_median),
      TRUE ~ median
    ),
    lci = case_when(
      zm_lake == "Zebra Mussel" & zm_sample_lag_2 == "y" ~ (lci - discount_lci),
      TRUE ~ lci
    ),
    uci = case_when(
      zm_lake == "Zebra Mussel" & zm_sample_lag_2 == "y" ~ (uci - discount_uci),
      TRUE ~ uci
    ),
    smoothed_median = case_when(
      zm_lake == "Zebra Mussel" & zm_sample_lag_2 == "y" ~ (smoothed_median - discount_smoothed_median),
      TRUE ~ smoothed_median
    ),
    smoothed_lci = case_when(
      zm_lake == "Zebra Mussel" & zm_sample_lag_2 == "y" ~ (smoothed_lci - discount_smoothed_lci),
      TRUE ~ smoothed_lci
    ),
    smoothed_uci = case_when(
      zm_lake == "Zebra Mussel" & zm_sample_lag_2 == "y" ~ (smoothed_uci - discount_smoothed_uci),
      TRUE ~ smoothed_uci
    )
  ) %>% 
  select(-starts_with("discount_")) %>%
  filter(zm_lake != "Reference") %>% 
  mutate(lgthin = lgthin*25.4)
  
#zebra mussel lakes are discounted by the change in uninvaded systems here so we don't need to display them
ggplot() +
  theme_bw() +
  geom_ribbon(data = pred_lengths_discount, aes(x = lgthin, y = median, color = zm_sample_lag_2, ymax = smoothed_uci, ymin = smoothed_lci, fill = zm_sample_lag_2), lwd = 0, alpha = .3) + # alpha is transparency
  geom_line(data = pred_lengths_discount, lwd = 1, aes(x = lgthin, y = median, color = zm_sample_lag_2, lty = zm_sample_lag_2)) + # center line of ribbon
  # geom_point(data = wae_data %>% 
  #              mutate(zm_lake = case_when(zm_lake == "y" ~ "Zebra Mussel",
  #                            zm_lake == "n" ~ "Reference"),
  #                     lgthin = lgthin*25.4) %>% 
               # filter(lgthin > 190 & lgthin < 525), aes(x = lgthin, y = hgppm, color = zm_sample_lag_2), alpha = .2) +
  scale_linetype_manual(values = c(1, 6), labels = c("Before", "After")) + # to tell it which line type you want
  scale_colour_manual(values = c("#00CCCC", "#FF6666"), labels = c("Before", "After")) +
  scale_fill_manual(values = c("#00CCCC", "#FF6666"), labels = c("Before", "After")) +
  ylab("Mercury Concentration (ppm)") +
...
pred_lengths %>% 
  ...
  group_by(zm_lake, zm_sample_lag_2) %>% 
  filter(distance == min(distance)) %>% 
  select(zm_lake, zm_sample_lag_2, lgthin) %>% 
  pivot_wider(names_from = c(zm_lake, zm_sample_lag_2), values_from = lgthin) %>% 
  mutate(diff_ref = Reference_y - Reference_n,
         diff_zm = `Zebra Mussel_y` - `Zebra Mussel_n`,
         dicount_diff = diff_zm - diff_ref)
###############################################################################
...
expand_grid(species = c("walleye", "perch", "pike"),
            )
...
ppm_length <- pike_length_ppm / walleye_length_ppm +
  plot_layout(height = c(1,1))
ppm_length
ggsave(plot = ppm_length, "length_ppm_wae_nop.jpg", height = 7, width = 7, dpi = 600, bg = "white")
...
species_summary_stats <- list(
  nop = nop_data %>% summarise(mean_length = mean(lgthmm), sd_length = sd(lgthmm)),
  wae = wae_data %>% summarise(mean_length = mean(lgthmm), sd_length = sd(lgthmm)),
  blc = blc_data %>% summarise(mean_length = mean(lgthmm), sd_length = sd(lgthmm)),
  blg = blg_data %>% summarise(mean_length = mean(lgthmm), sd_length = sd(lgthmm)),
  lmb = lmb_data %>% summarise(mean_length = mean(lgthmm), sd_length = sd(lgthmm)),
  yep = yep_data %>% summarise(mean_length = mean(lgthmm), sd_length = sd(lgthmm))
)


#get summary data from model outputs
species_summary <- species_lengths %>%
  mutate(summary_data = map2(species, standard_length, function(sp, len) {
    
    # gather summary stats for scaled length back calculation
    length_stats <- species_summary_stats[[sp]]
    
    specific_length_scaled <- (len - length_stats$mean_length) / length_stats$sd_length
    print(specific_length_scaled)

    # Extract marginal effects from the model
    marg_draws <- emmeans(get(paste0("fit_", sp)), ~ zm_lake * zm_sample_lag_2,
                          at = list(scaled_length = specific_length_scaled),
                          re_formula = NA, epred = TRUE) %>%
      gather_emmeans_draws() %>%
      mutate(hg_ppm = exp(.value))
    print(head(marg_draws))
    ...
    marg_draw_plot <- marg_draws %>%
      ...
      interaction = factor(interaction, levels = c("Uninvaded Before", "Uninvaded After", "Zebra Mussel Before", "Zebra Mussel After")))
    print(unique(marg_draw_plot$interaction))
    print(get(paste0("fit_", sp)))
    ...
    return(list(
      ...
      median_lines_plot = median_lines_plot,
      diff_plot_data = diff_plot_data
    ))
  }))

#species summary contains all of the data we need to create plots, just have to find those data

#to save space in my memory, I am going to remove unneeded, large objects
rm(fit_blc,
   fit_blg,
   fit_lmb,
   fit_nop,
...
#combined diff of diff
combined_diff_of_diff <- species_summary %>%
  mutate(diff_of_diff = map2(species, summary_data, function(sp, data) {
    data$diff_plot_data %>%
      mutate(species_diff = sp)  # Add the species column to the difference of difference data
  })) %>% 
  unnest(diff_of_diff)


#lets first just plot the difference of difference for the species 
diff_of_diff_data <- species_summary %>% 
   mutate(diff_of_diff = map(summary_data, ~ .x$diff_plot_data %>%
    filter(zm_lake == "Difference in Change"))) %>%
  select(species, diff_of_diff) %>%
  unnest(diff_of_diff)

plot2 <- data$diff_plot_data %>% 
      ggplot() +
      stat_pointinterval(aes(x = zm_lake, y = value, color = zm_lake), .width = c(.66, .95)) +
      geom_hline(yintercept = 0, linetype = "dashed") +
      ylab("Difference in Mercury Concentration\nBefore and After (ppm)") +
...
#generate plots
species_plots <- species_summary %>%
  mutate(plot = map(summary_data, function(data) {
    
    plot1 <- data$marg_draw_plot %>%
      ggplot(aes(x = interaction, y = hg_ppm, color = interaction, shape = interaction)) +
      stat_pointinterval(.width = c(.66, .95), show.legend = F)  +
      scale_shape_manual("Sample Invasion",
                         values = c(1, 16, 1, 16)) +
      geom_line(
        data = data$median_lines_plot,
        aes(x = interaction, y = median_hg_ppm, group = zm_lake),
        position = position_dodge(width = 0.1), # Align lines with intervals
        inherit.aes = FALSE,
        key_glyph = "rect",
...
wrap_plots(species_plots$plot, ncol = 2)

#plot with just the differences 
all_diff <- species_summary %>%
  transmute(diff_plot_data = map2(species, summary_data, ~ mutate(.y$diff_plot_data, species = .x))) %>%
  unnest(diff_plot_data)
...
all_diff %>% 
  mutate(species = case_when(species == "blc" ~ "Black Crappie",
         ...
         species = factor(species, levels = c( "Northern Pike", 
                                               ...
                                               "Bluegill"))) %>% 
  ggplot() +
  stat_pointinterval(aes(x = species, y = value, color = species ), .width = c(.66, .95)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  ylab("Difference in Mercury Concentration\nBefore and After (ppm)") +
  xlab("") +
  theme_minimal() +
...
# PUlling Leech Lake Random Effects
```{r}
#11014700 - winni
#04003000 - cass
#11020300 - Leech
...
#raw plots
combined_data %>%
  filter(dow %in% c("11014700", "04003000", "11020300")) %>% 
  mutate(spec = case_when(spec == "WAE" ~ "Walleye",
                          spec == "NOP" ~ "Northern Pike"),
         waterway = case_when(waterway == "WINNIBIGOSHISH" ~ "WINNI",
                              TRUE ~ waterway)) %>% 
  ggplot() +
  geom_point(aes(lgthmm, hgppm, color = zm_sample_lag_2)) +
  geom_smooth(aes(lgthmm, hgppm, color = zm_sample_lag_2), se = F, method = "lm", formula = y ~ poly(x, 2)) +
  geom_hline(yintercept = 0.22, linetype = "dashed") +
  # geom_vline(data = tibble(spec = c("Walleye", "Northern Pike"), lgthmm = c(381, 600)), 
  #            aes(xintercept = lgthmm), linetype = "dotted", color = "black") +
  scale_color_manual("", values = c("lightblue", "salmon"), labels = c("Pre-Invasion", "Zebra Mussel")) +
  labs(y = "Mercury Concentration (ppm)",
       x = "Length of Fish (mm)") +
  facet_grid(waterway ~ spec, scales = "free") +
  theme_minimal() +
  theme(legend.position = "bottom")
ggsave("raw_data_poly_fit.jpg")

#random effects
length_seq <- seq(min(wae_data$scaled_length), max(wae_data$scaled_length), length.out = 25) 

pred_data <- wae_data %>% 
  filter(dow %in% c("11014700", "04003000", "11020300")) %>%
  select(dow, zm_lake, zm_sample_lag_2, yearcoll) %>%
  distinct() %>%
  expand_grid(scaled_length = length_seq)

predition_data_wae <- pred_data %>% 
  add_epred_draws(fit_wae) %>% 
  median_qi(lake.median = exp(.epred), .width = c(.95)) 

predition_data_wae %>% 
  ggplot() +
  geom_point(aes(scaled_length, lake.median, color = zm_sample_lag_2)) +
  geom_smooth(aes(scaled_length, lake.median, color = zm_sample_lag_2)) +
  scale_color_manual("", values = c("lightblue", "salmon"), labels = c("Pre-Invasion", "Zebra Mussel")) +
  facet_grid(~dow) +
  theme_minimal() +
  theme(legend.position = "bottom") 
...
#pike
length_seq <- seq(min(nop_data$scaled_length), max(nop_data$scaled_length), length.out = 25) 
...
predition_data_nop <- pred_data %>% 
  add_epred_draws(fit_nop) %>% 
  median_qi(lake.median = exp(.epred), .width = c(.95)) 
...
#combined
length_stats <- combined_data %>% 
  group_by(spec) %>% 
  summarise(mean_length = mean(lgthmm),
            sd_length = sd(lgthmm)) %>% 
  mutate(spec = case_when(spec == "NOP" ~ "Northern Pike",
                          spec == "WAE" ~  "Walleye",
                          TRUE ~ NA))
  

pred_nop_wae <- bind_rows(predition_data_nop %>% 
                            mutate(spec = "Northern Pike"), 
                          predition_data_wae %>% 
                            mutate(spec = "Walleye"))  %>% 
  mutate(waterway = case_when(dow == "11020300" ~ "LEECH",
                               dow == "04003000" ~ "CASS",
                               dow == "11014700" ~ "WINNIBIGOSHISH")) %>% 
  left_join(length_stats) %>% 
  mutate(lgthmm = scaled_length*sd_length + mean_length)

#update this plot with the actual raw data in the background 
#for this plot, the modeled lines can be constructed of more than just the observed lengths 
ggplot() +
  geom_point(data = combined_data %>% 
                mutate(spec = case_when(spec == "NOP" ~ "Northern Pike",
                          spec == "WAE" ~  "Walleye",
                          TRUE ~ NA)) %>% 
               filter(dow %in% c(pred_nop_wae$dow)), aes(lgthmm, hgppm, color = zm_sample_lag_2)) +
  geom_smooth(data = pred_nop_wae, aes(lgthmm, lake.median, color = zm_sample_lag_2), se = F) +
  geom_hline(yintercept = 0.22, linetype = "dashed") +
  scale_color_manual("", values = c("lightblue", "salmon"), labels = c("Pre-Invasion", "Zebra Mussel")) +
  labs(x = "Length of Fish (mm)",
       y = "Mercury Concentration (ppm)") +
  facet_grid(waterway ~spec, scales = "free") +
  theme_minimal() +
  theme(legend.position = "bottom") 
ggsave("modeled_random_effects.jpg")
```
...
#creating data for entire baci analysis 
combined_data <- bind_rows(nop_data,
                           wae_data,
                           blg_data,
                           blc_data,
                           lmb_data,
...
write_csv(combined_data, "combined_hg_data.csv")

#quick check 
combined_species <- bind_rows(nop_data, wae_data)
combined_species %>% 
  filter(zm_lake == "n") %>% 
  count(dow)
