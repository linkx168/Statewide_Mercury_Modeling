...
*code needs to be updated for new model run 1/16
```{r}
...
#re_formula = NA avoids species specific intercepts 
...
ggplot() +
  ...
  geom_point(data = nop_data, aes(x = lgthin, y = hgppm, color = zm_sample_lag_2), alpha = .2) +
...
#clear objects 
rm(combined_plot, contrasts_draws, fit_nop, marg_draws, median_lines, pike_outline, plot1, plot2, diff_lakes, diff_of_diff,
   diff_plot_data, median_lines_plot)
```
...
#######################changing axis############################################
marg_draw_plot <- marg_draws %>% 
  mutate(interaction = paste0(zm_lake, zm_sample_lag_1)) %>% 
  mutate(interaction = case_when(interaction== "nn" ~ "Uninvaded Before",
                                 interaction == "ny" ~ "Uninvaded After",
                                 interaction == "yn" ~ "Zebra Mussel Before",
                                 interaction == "yy" ~ "Zebra Mussel After",
...
#clear objects 
rm(combined_plot, contrasts_draws, fit_wae, marg_draws, walleye_outline, plot1, plot2)
```
