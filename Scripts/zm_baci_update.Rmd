...
species_data %>% 
  ...
  summarise(n_years = n_distinct(yearcoll))
#looks like each level of zm_lake has all 23 years represented but they are not all repped at the sample level
...
# Analysis 
Be sure to change the zm_sample variable to the specified level of lag set in the filtering script, change to the desired lag
```{r}
#walleye model fit
...
contrasts <- contrast(emmeans(fit_nop, ~ zm_lake * zm_sample_lag_1, re_formula = NA, epred = TRUE, at = list(scaled_length = -1.599011)), 
                      method = "revpairwise", by = "zm_lake")
contrasts
rm(contrasts)
...
contrasts_draws %>% 
  ...
  summarise(n_larger = sum(diff_of_diff > 0),
            ...
            median = median(diff_of_diff),
            mean = mean(diff_of_diff),
            ci_95_lower_diff = quantile(diff_of_diff, probs = c(0.025)),
            ci_95_upper_diff = quantile(diff_of_diff, probs = c(0.975)))
...
#sensitivity analysis of lag year - median year (no matching)
```{r}
#creating function that is flexible to the lag variable used
create_species_data <- function(species, data, lag_variable) {
  # Filter zebra mussel lakes using the specified lag variable
  zm_lakes <- data %>% 
    filter(spec == species) %>% 
    filter(yearcoll >= 2000) %>% 
    filter(zm_lake == "y") %>% 
  ...
  # Filter reference lakes (unchanged strategy)
  ref_lakes <- data %>% 
    filter(spec == species) %>% 
    filter(yearcoll >= 2000) %>% 
    filter(zm_lake == "n") %>% 
    mutate(!!sym(lag_variable) := case_when(
      yearcoll >= 2011 ~ "y",
      yearcoll <= 2010 ~ "n"
    )) %>% 
    distinct(dow, !!sym(lag_variable)) %>% 
...
# looping over the lag variable for each species 
species <- c("WAE", "NOP")
...
# looping over a model for sensitivity analysis
# Loop through species and lag variables to fit models
for (species in species) {
  for (lag_var in lag_variables) {
...
# Specify the object(s) to keep
objects_to_keep <- c("hg.model")

# Remove all objects except those specified in `objects_to_keep`
rm(list = setdiff(all_objects, objects_to_keep))
rm(all_objects, objects_to_keep)
```

#More advanced filter - matching
choose your spaces, data, and lag sample variable 
```{r}
create_species_data <- function(species, data, lag_variable) {
  # Filter zebra mussel lakes
  zm_lakes <- data %>% 
  ...
  ref_lakes <- ref_lakes %>% 
    mutate(nearest_zm_lake = st_nearest_feature(geometry, zm_lakes$geometry),
           ...
           !!sym(lag_variable) := case_when(yearcoll >= nearest_infested_year  ~ "y",
                                       yearcoll <= nearest_infested_year ~ "n")) %>% 
    distinct(dow, !!sym(lag_variable)) %>% 
    group_by(dow) %>% 
    count() %>% 
    filter(n >1)
...
zm_lakes <- hg.model %>% 
  ...
  ref_lakes <- ref_lakes %>% 
    mutate(nearest_zm_lake = st_nearest_feature(geometry, zm_lakes$geometry),
           nearest_infested_year = zm_lakes$year_infested[nearest_zm_lake],
           zm_sample_lag_1 := case_when(yearcoll >= nearest_infested_year  ~ "y",
                                       yearcoll <= nearest_infested_year ~ "n")) %>% 
    distinct(dow, zm_sample_lag_1) %>% 
    group_by(dow) %>% 
    count() %>% 
  ...
  ref_fish <- hg.model %>% 
    ...
    mutate(nearest_zm_lake = st_nearest_feature(geometry, zm_lakes$geometry),
           nearest_infested_year = zm_lakes$year_infested[nearest_zm_lake],
           zm_sample_invasion_lag_1 := case_when(yearcoll >= nearest_infested_year  ~ "y",
                                       yearcoll <= nearest_infested_year ~ "n")) 
  
  data <- bind_rows(ref_fish, zm_fish) %>% 
    st_drop_geometry() %>% 
    mutate(centroid_lat = centroid_lat_orig,
           centroid_lon = centroid_lon_orig) %>% 
    select()
