...
wae_pred <- pred_data %>% 
  filter(spec == "WAE")
check <- fit_wae %>% 
  add_predicted_draws(newdata = wae_pred, re_formula = NA)  %>% 
  mutate(invasion = paste0(zm_lake, zm_sample_lag_2)) %>% 
  select(invasion, scaled_length, .draw, .prediction) %>% 
  ungroup() %>% 
  pivot_wider(id_cols = c(.draw, scaled_length),names_from = invasion,values_from = .prediction) %>% 
  mutate(correction = ny - nn,
         yy_corrected = yy-correction)

#############4/1 - stopped here####################
#I am really close - next steps
#add in background correction to zm invasion
#run the function for all spec
#create plot of before and after with background correction with icons
#then move onto ppm vs length 
...
ggplot() +
  geom_line(data = combined_exceed, aes(x = lgthmm, y = prob, color = zm_sample_lag_2)) + 
  ...
  facet_grid(spec~zm_lake, scales = "free_x") +
