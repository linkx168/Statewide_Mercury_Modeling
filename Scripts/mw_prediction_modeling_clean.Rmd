...
ggplot() +
  geom_sf(data = states, fill = "white", color = "black") +
  geom_sf(data = data_for_map, aes(size = n_samples), shape = 21, color = "blue", fill = "orange", alpha = .7) +
  scale_size(name = "Number of Samples") +
  facet_wrap(~species) +
  theme_minimal() +
...
left_out_scaled <- left_out_scaled %>% 
  mutate(species_case = case_when(lake_status == "new" ~ "new_lake",
                                  map_lgl(species_present, ~ !("walleye" %in% .x | "northern_pike" %in% .x)) ~ "partial_no_wae_nop",
...
lake_spec_sum <- lake_spec_pred %>% 
  ...
  mutate(back_transformed = exp(.prediction)) %>% 
  group_by(lagos_id, species) %>% 
  summarise(
    log_hgppm = median(.prediction),
    log_lower = quantile(.prediction, 0.05),
    log_upper = quantile(.prediction, 0.95),
