...
pred_grid <- lake_spec_pred %>% 
  ...
  distinct(lagos_id, 
           ...
           scaled_logit_forcon,
           scaled_logit_fordec) %>% 
  left_join(length_grid) %>% 
  add_epred_draws(fit, ndraws = 500, allow_new_levels = T) %>% 
...
glimpse(lake_spec_sum)

ggplot(lake_spec_sum,
       aes(x = factor(lagos_id),
           y = species,
           fill = hgppm)) +
  ...
  scale_fill_gradient2(
    ...
    high = "red",
    midpoint = 0.22,
    name = "Predicted Hg (ppm)"
  ) +
  geom_text(
    aes(label = scales::percent(prob_over, accuracy = 1)),
    size = 3
  ) +
  labs(x = "Lake", y = "Species",
       caption = "Text shows P(Hg > 0.22 ppm)") +
  theme_minimal()
...
ggplot(
  ...
  theme_minimal()
#ggsave("leech_over_22.jpg", width = 5, height = 5, dpi = 600)
```
