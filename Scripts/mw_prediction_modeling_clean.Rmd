...
# data
```{r}
mw_hg <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Data", "hg_data_comb_12DEC2025.csv"))
```
...
rm(pred_data, pred_spec, lake_link, lake_scalers, length_scalers)
#############################################################################
fit <- readRDS(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output", "mw_random_leave_out.rds"))
...
lake_spec_pred <- pred_full %>% 
  add_predicted_draws(fit, ndraws = 100, allow_new_levels = TRUE)
rm(fit, pred_full)
...
lake_spec_sum <- lake_spec_pred %>% 
  ...
  summarise(
    ...
    prob_over = mean(.prediction > 0.22),
    .groups = "drop"
  ) 
rm(lake_spec_pred)
...
lake_spec_org <- lake_spec_sum %>% 
  mutate(risk = case_when(prob_over >= 0.22 ~ "high",
