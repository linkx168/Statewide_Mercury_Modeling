...
mw_hg_prep <- mw_hg_prep %>% 
  ...
  #dealing with LOD - taking half of the LOD - this method should hold up, but take closer look if LOD value is consistent *removed on 4/8 per eagles smith 2016*
  # mutate(hg_ppm = case_when(hg_ppm <= 0.01 ~ hg_ppm/2,
  #                          TRUE ~ hg_ppm)) %>% 
  #correct whole body samples to fillets as eagle-smith 2016
  mutate(hg_ppm = case_when(sample_type %in% c("WHORG", "WHOLE FISH") ~
                              hg_ppm/0.74,
                            TRUE ~ hg_ppm)) %>% 
...
#composite samples
mw_hg_prep %>% 
  summarise(na = sum(is.na(num_fish_in_sample)),
            ind_record = sum(num_fish_in_sample == 1, na.rm = T),
            comp_sample = sum(num_fish_in_sample > 1, na.rm = T),
            total_fish = n(),
            prop_na = na/total_fish,
            prop_comp = comp_sample/total_fish,
            prop_ind = ind_record/total_fish)
...
#sample types
mw_hg_prep %>% 
  distinct(state, sample_type)

mw_hg_prep %>% 
  mutate(sample_type_clean = tolower(sample_type),
         sample_type_clean = case_when(sample_type_clean == "skin on fillet" ~ "filsk",
                                       sample_type_clean == "whole fish" ~ "whorg",
                                       sample_type_clean == "skin off fillet" ~ "filet",
                                       sample_type_clean == "skin on fillet" ~ "filsk",
                                       TRUE ~ sample_type_clean)) %>% 
...
#limit of detection
#start with MN because they give us info in metadata
mw_hg_prep %>% 
  filter(hg_code == "K") %>% 
  group_by(state, hg_ppm) %>% 
  count()
...
mw_hg_prep %>% 
  ...
  ggplot() +
  geom_histogram(aes(hg_ppm), binwidth = 0.001) +
  geom_histogram(data = mw_hg_prep %>% 
                   filter(hg_ppm == 0.01), aes(x = hg_ppm), fill = "yellow", binwidth = 0.001) +
  facet_wrap(~state)
#okay the only state with a clear LOD appears to be MN with 0.01, with no inferred LOD from the data 

mw_hg_prep %>% 
  summarise(n_fish = n(),
            n_under = sum(hg_ppm <= 0.01),
            prop_under = n_under/n_fish)


#how many lakes by state
mw_hg_prep %>% 
  group_by(state) %>% 
...
```{r}
 # write_csv(hg.data.loo.mw, file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Data", "hg_model_data_loo_mw.csv"))
...
left_out_data <- mw_hg_prep %>% 
  mutate(hg_ppm_join = round(hg_ppm, 10)) %>% 
  anti_join(hg.data.loo_anti %>% 
              mutate(hg_ppm_join = round(hg_ppm, 10)), by = c("lagos_id",
                                                              "year",
                                                              "species",
                                                              "length_in",
                                                              "hg_ppm_join")) %>% 
...
ggplot() +
  geom_sf(data = states, fill = "white", color = "black") +
  geom_sf(data = subset(data_for_map, data == "Training"), aes(size = n_samples, color = data), alpha = .5) +
  geom_sf(data = subset(data_for_map, data == "Left Out"), aes(size = n_samples, color = data), alpha = .5) +
...
#map without n scaling 
ggplot() +
  geom_sf(data = states, fill = "white", color = "black") +
  geom_sf(data = subset(data_for_map, data == "Training"), aes(color = data), size = 0.5, alpha = .5) +
  geom_sf(data = subset(data_for_map, data == "Left Out"), aes(color = data), size = 0.5, alpha = .5) +
  scale_color_manual("Data", values = c("Training" = "navy", 
                                "Left Out" = "orange")) +
  facet_wrap(~species) +
  guides(color = guide_legend(direction = "horizontal"),
...
analysis_total %>% 
  summarise(n_distinct(lagos_id, species))
...
#fish size summaires
fish_size <- analysis_total %>% 
  group_by(species, data) %>% 
  summarise(min = min(length_in),
            mean = mean(length_in),
            max = max(length_in),
            sd = sd(length_in),
            n = n())
#write_csv(fish_size, "fish_size.csv")
rm(fish_size)
```
...
# saveRDS(fit, file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output", "mw_random_leave_out.rds"))
...
# write_csv(predictions, file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output",  "predictions_left_out_mw.csv"))
...
# I imagine our values are pretty close to 0.22 when we miss - is that the case?
lake_species_compare %>% 
  mutate(obs_harmful = case_when(mean_obs >= 0.22 ~ "y",
                                 TRUE ~ "n"),
         pred_harmful = case_when(mean_pred >= .22 ~ "y",
                                  TRUE ~ "n")) %>% 
  filter(obs_harmful != pred_harmful) %>% 
  ggplot() +
  geom_histogram(aes(mean_obs)) +
...
lake_species_compare %>% 
  mutate(obs_harmful = case_when(mean_obs >= 0.22 ~ "y",
         ...
         pred_harmful = case_when(mean_pred >= .22 ~ "y",
                                  TRUE ~ "n")) %>% 
  filter(obs_harmful != pred_harmful) %>%
  summarise(mean = mean(mean_obs),
            med = median(mean_obs),
            min = min(mean_obs),
            max = max(mean_obs),
            avg_dist_away = mean(mean_obs - 0.22),
            num_under_05 = sum(abs(mean_obs - 0.22) <= 0.05),
            num_under_1 = sum(abs(mean_obs - 0.22) <= 0.1),
            prop_under_05 = sum(abs(mean_obs - 0.22) <= 0.05)/ n(),
            prop_under_1 = sum(abs(mean_obs - 0.22) <= 0.1)/ n())
...
lake_species_compare %>% 
  ...
  filter(obs_harmful != pred_harmful) %>%
  filter(mean_obs > 0.5)
#the biggest prediction is a blg at 0.6 - no wonder why we didn't predict that! - maybe a typo in the raw data?
#funny enough this is the consumption advisory only for bullhead lake 
...
#lake link
lake_link <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Data", "Covariate Data", "lake_link.csv")) %>% 
...
#Keep none-MI lakes OR keep counties listed in the UP
covary <- covary %>% 
  filter(!str_detect(ws_states, "MI") | lake_county %in% up_counties)

covary %>% 
  group_by(ws_states) %>% 
  summarise(n_counties = n_distinct(lake_county))
#############################################################################
pred_data <- covary %>% 
...
# write_csv(lake_spec_sum, file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output", "lake_spec_hg_at_harvest.csv"))

lake_spec_sum <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output", "lake_spec_hg_at_harvest.csv"))
###################making map#############################
...
ggplot() +
  geom_sf(data = states, fill = "white", color = "black") +
  geom_sf(
    data = lake_sf, aes(color = prob_over), alpha = .1, size = 0.2) +
  facet_wrap(~ species) +
...
ggplot() +
  ...
  scale_color_manual("Risk for Consumption", 
                     values = c("#0D0887", "#9C179E", "#ED7953", "#F0F921"),
                     labels = c("High", "Worth a Check", "Yellow Flag", "Low")) +
...
lake_spec_sum %>% 
  ...
  scale_fill_manual("Consumption Risk",
                    values = c("#0D0887", "#9C179E", "#ED7953", "#F0F921"),
                    labels = c("High", "Risky", "Less Risky", "Low")) +
  labs(x = NULL,
       y = NULL) +
  theme_minimal() +
  theme(legend.position = "bottom",
...
rm(lake_spec_sum, lake_sf, lagos_watershed, lagos_watershed_lulc, xwak)
```
...
coef_df <- as.data.frame(fixef(fit)) %>%
  rownames_to_column("term")

coef_df2 <- coef_df %>%
  filter(
    str_detect(term, "scaled_"),
    !str_detect(term, ":"),
    term != "scaled_length"
  )

p_cov <- ggplot(coef_df2,
       aes(
         x = Estimate,
...
draws <- spread_draws(
  fit,
  b_scaled_length,
  `b_scaled_length:speciesbluegill`,
  `b_scaled_length:specieslargemouth_bass`,
  `b_scaled_length:speciesnorthern_pike`,
  `b_scaled_length:specieswalleye`,
  `b_scaled_length:specieswhite_sucker`,
  `b_scaled_length:speciesyellow_perch`
)

# compute species-specific slopes
species_slopes <- draws %>%
  transmute(
    .draw,

    `Black Crappie` = b_scaled_length,
...
ggsave("beta_plot.jpg", height = 5, width = 5, dpi = 600)

################more complex covarite plot#####################################
#this gets at "what lakes are most risky"
...
#saving results
# write_csv(results, file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output", "length_crossing_by_lake.csv"))

results <- read_csv(file.path("D:",
                              "Shared drives",
                              "Hansen Lab",
                              "RESEARCH PROJECTS",
                              "Statewide mercury modeling",
                              "Predictive Modeling",
                              "Random Model Final Output", 
                              "length_crossing_by_lake.csv"))
...
plot_df <- sum_length %>%
  left_join(pred_spec, by = "species") %>%
  mutate(risk = harvest_length > mean)

ggplot(plot_df, aes(y = reorder(species, harvest_length))) +
  ...
  # line connecting threshold to harvest (the "risk gap")
  geom_segment(
    aes(x = mean, xend = harvest_length, yend = species,
        color = risk),
    size = 1.2,
    alpha = 0.5,show.legend = F
  ) +
  
  # threshold point (mean)
  geom_point(aes(x = mean), size = 3) +
  
  # harvest point
  geom_point(
    aes(x = harvest_length),
    shape = 21,
    fill = "white",
    size = 3,
    stroke = 1
  ...
  theme_bw()
#ggsave("region_length_avg_color.jpg", height = 5, width = 5, dpi = 600)
...
plot_df <- final_results2 %>%
  left_join(pred_spec, by = "species") %>%
  mutate(
    risk = if_else(
      length < harvest_length,
      "Crosses before harvest",
      "Harvest before crossing"
    )
  )
rm(final_results2)
...
species_order <- plot_df %>%
  distinct(species, harvest_length) %>%
  arrange(harvest_length) %>%
  pull(species)

plot_df$species <- factor(plot_df$species, levels = species_order)
pred_spec$species <- factor(pred_spec$species, levels = species_order)
...
ggplot(plot_df,
  ...
  geom_point(
    data = pred_spec,
    aes(x = species, y = harvest_length),
    shape = 124,
    size = 12,
    color = "black",
    inherit.aes = FALSE
  ...
  scale_color_manual(
    values = c(
      "Crosses before harvest" = "salmon",
      "Harvest before crossing" = "lightblue"
    )
  ) +
  ...
  labs(
    x = NULL,
    y = "Length where fish exceed 0.22 ppm Hg",
    fill = NULL,
    color = NULL
  ) +
...
ggsave("harvest_length_violin.jpg", height = 5, width = 5, dpi = 600)
...
# hg predicition by length
-generate predictions for each lake-species for 10 lengths for all lakes based on 200 draws (expected values)
   -these are expected value, so the credible intervals will represent varaition on the average fish for a given length, not the variation of "any particular fish"
```{r}
# for curve plotting
length_grid <- tibble(
  species = c("walleye","northern_pike","black_crappie",
              "largemouth_bass","bluegill","white_sucker","yellow_perch")
) %>%
  left_join(length_scalers, by = "species") %>%
  rowwise() %>%
  mutate(length_seq = list(seq(
    length_mean - 2*length_sd,
    length_mean + 2*length_sd,
    length.out = 10
  ))) %>%
  unnest(length_seq) %>%
  rename(length = length_seq) %>%
  mutate(scaled_length = (length - length_mean)/length_sd)

#combine lengths with lake-spec
curve_pred_data <- crossing(pred_data, length_grid)

curve_draws <- curve_pred_data %>% 
  add_epred_draws(fit, ndraws = 200, allow_new_levels = T)

curve_summary <- curve_draws %>%
  mutate(hgppm = exp(.epred)) %>%
  group_by(lagos_id, species, length) %>%
  summarise(
    hg = median(hgppm),
    lower = quantile(hgppm, 0.1),
    upper = quantile(hgppm, 0.9),
    .groups = "drop"
  )
rm(fit, curve_draws)
#saveRDS(curve_summary, "curve_summary.rds")

#check on example (Leech)
curve_summary %>% 
  filter(lagos_id == 996) %>% 
  ggplot() +
  geom_point(aes(length, hg, color = species)) +
  geom_smooth(aes(length, hg, color = species)) +
  geom_ribbon(aes(length, hg, ymin = lower, ymax = upper, color = species, fill = species),
              alpha = 0.2) +
  geom_hline(yintercept = 0.22, linetype = "dashed")
```

## lake-spec data level
```{r}
#lake-species we predicted
species_list <- c(
  "walleye",
  "northern_pike",
  "black_crappie",
...
pred_lake_spec <- crossing(lagos_id = pred_data$lagos_id,
                           species = species_list) %>% 
  mutate(lagos_id = as.character(lagos_id))

#lake-species we observed
obs_lake_species <- hg.data.loo.mw %>% 
  group_by(lagos_id, species) %>% 
  summarise(n = n()) %>% 
  mutate(lagos_id = as.character(lagos_id),
         has_species_data =T)

obs_lakes <- hg.data.loo.mw %>% 
  distinct(lagos_id) %>% 
  mutate(has_any_data = T) %>% 
  mutate(lagos_id = as.character(lagos_id))

lake_data_level <- pred_lake_spec %>% 
  left_join(obs_lake_species, by = c("lagos_id", "species")) %>% 
  left_join(obs_lakes, by = c("lagos_id")) %>% 
  mutate(data_level = case_when(is.na(has_any_data) ~ "none",
                                !is.na(has_species_data) ~ "species",
                                TRUE ~ "other_species")) %>% 
  select(lagos_id,
         species,
         data_level,
...
#gather some other information to display
xwalk <- mw_hg %>% 
  group_by(lagos_id) %>% 
  summarise(latest_year = max(year),
            state_id = paste(unique(state_id), collapse = ", "),
            lake_name = paste(unique(lake_name), collapse = ", "),
            .groups = "drop") %>% 
  mutate(lagos_id = as.character(lagos_id))

xwalk_unobserved <- pred_data %>% 
  left_join(lake_link %>% 
              rename(lagos_id = lagoslakeid) %>% 
              mutate(state = case_when(lake_centroidstate == "MN" ~ "Minnesota",
                                       lake_centroidstate == "WI" ~ "Wisconsin",
                                       TRUE ~ lake_centroidstate))) %>% 
  select(lagos_id,
    lake_name = lake_namegnis,
         state,
         lat_decdeg = ws_lat_decdeg,
         ...
         nlcd_wetwood90_pct) %>% 
  mutate(lagos_id = as.character(lagos_id))
#we could do a local to lagos here to retrive state ids but there will be issue with multipule state ids within a lagos, but lets skip for now
all_lakes <- xwalk_unobserved %>% 
  left_join(xwalk)
rm(xwalk, xwalk_unobserved)

#add this to the other lake data
lake_data_level <- lake_data_level %>% 
  left_join(all_lakes)

#we currently don't include the MI lakes
saveRDS(lake_data_level, "lake_data_level.rds")
rm(obs_lake_species,
   obs_lakes,
   pred_lake_spec,
   lake_data_level)
...
## length thresholds
```{r}
ln_x <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output", "length_crossing_by_lake.csv"))
...
ln_x %>% 
  #leech lake
  filter(lagos_id == "996")
saveRDS(ln_x, "length_crossing.rds")
```
