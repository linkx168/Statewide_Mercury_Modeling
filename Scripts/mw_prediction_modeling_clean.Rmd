...
#limit of detection
#start with MN becuase they give us info in metadata
mw_hg_prep %>% 
  filter(hg_code == "K") %>% 
  group_by(hg_ppm) %>% 
  count()
...
mw_hg_prep %>% 
  ...
  geom_histogram(data = mw_hg_prep %>% 
                   filter(hg_ppm == 0.005), aes(x = hg_ppm), fill = "yellow", binwidth = 0.001) +
  facet_wrap(~state)
#okay the only state with a clear LOD appears to be MN with 0.01 (0.005 here after correction), all other states not having one
