...
```{r}
hg_explore <- read_csv("Data/allfish_data_20231102_IT.csv") %>% 
  filter(!(is.na(LGTHIN) | is.na(HGPPM) | is.na(DOWID))) %>% #removes samples with no hg, length, or dow data
  mutate(DOWID = fixlakeid(DOWID)) 
...
#what is the limit dected?
hg %>% 
  filter(str_detect(HGCODE, "K")) %>% 
  group_by(HGPPM) %>% 
  count() %>% 
  print(n = nrow(.))
#looks like it is generally .01-.05 but there might be some erroneous values? .551 certainly is not a limit of detection
...
hg %>% 
  ...
  count() %>% 
  print(n = nrow(.))
#it appears most of the erroneus values are from 2022 - sometimes there is variation in the LOD within a year even
...
cor_long %>% 
  filter(Var1 %in% c("NOP", "WAE", "SMB", "LMB", "BLG", "WTS", "CAP", "YEP", "BLC", "CCF", "CIS", "TLC"),
         Var2 %in% c("NOP", "WAE", "SMB", "LMB", "BLG", "WTS", "CAP", "YEP", "BLC", "CCF", "CIS", "TLC"),
         value > 0) %>% 
...
hg_zm %>% 
  ...
  group_by(zm_lake) %>% 
  count()
#213 zm lakes in the hg data 
# 1247 uninvaded lakes
...
hg_zm %>% 
  ...
  count()
#125 of the 214 zm lakes have pre and post data
...
#35 lakes have at least 2 years of post and 1 year of pre
#26 lakes have at least 2 years pre and post
#only 11 lakes have more than 3 years post data and at least 1 pre
