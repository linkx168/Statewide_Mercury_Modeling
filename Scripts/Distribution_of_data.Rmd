...
# looking at covarites
Find the metadata for the lagos data here
https://portal.edirepository.org/nis/metadataviewer?packageid=edi.854.1
-the zoneid is equvilate to the lagos id

```{r}
#first lets readin all of the relivent data
...
##############################################################################
#creating crosswalk for fish lakes in the CASC database
...
mw_data %>% 
   filter(!is.na(lake_name) & !is.na(nhdhr_id) & !is.na(lake_id)) %>% 
   filter(state == "Minnesota") %>% 
   distinct(lake_name, nhdhr_id, lake_id) %>% 
   collect() %>% 
   filter(str_detect(lake_name, "tonka")) %>% 
   distinct(lake_name, nhdhr_id, lake_id)

lake_ids <- mw_data %>% 
  select(state, nhdhr_id, lake_id) %>% 
  filter(!is.na(nhdhr_id) & !is.na(lake_id)) %>% 
  distinct(state, nhdhr_id, lake_id) %>% 
  collect() 
rm(mw_data)
...
lagos_watershed %>% 
  select(lagoslakeid,
         ...
         ws_states)
##########################################################################
#quick look at where there is not landcoverdata
bad <- lagos_watershed %>% 
...
covary <- lagos_watershed %>% 
  ...
  mutate(state_keep = case_when(str_detect("MN", ws_states) ~ "y",
                                str_detect("WI", ws_states) ~ "y",
                                str_detect("MI", ws_states) ~ "y",
                                TRUE ~ "n")) %>% 
  filter(state_keep == "y")
...
#############################################################################
#lets use the lagos crosswalk to get to nhdhrid to then connect to fish lakes
xwalk <- xwalk %>% 
  distinct(lagoslakeid,
         lake_nhdid,
         lake_namegnis,
...
rm(xwalk)
############################################################################
#now lets connect to fish lakes
lake_ids <- lake_ids %>% 
  rename(lake_nhdid = nhdhr_id) %>% 
  mutate(lake_nhdid = gsub("nhdhr_", "", lake_nhdid))

covary <- covary %>% 
  left_join(lake_ids, relationship ="one-to-many")
rm(lake_ids)
#############################################################################
#final_product
covary <- covary %>% 
...
covary <- covary %>% 
  mutate(state = case_when(ws_states == "MN" ~ "Minnesota",
                           ws_states == "WI" ~ "Wisconsin",
                           TRUE ~ "Michigan")) %>% 
  select(-ws_states)
...
covary <- covary %>% 
  ...
  select(-lake_namegnis,
         -lake_namelagos) %>% 
  relocate(lake_name, .after = lake_id)
