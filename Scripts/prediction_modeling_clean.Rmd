...
#infested waters list
zm <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Data", "Covariate Data", "ZM_MN_NHD.csv")) %>% 
  filter(!is.na(DOW))  %>% 
...
```{r}
hg.data.loo <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Data", "hg_model_data_loo.csv"))

hg.data.loo_anti <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Data", "hg_model_data_loo.csv")) %>% 
...
fit <- readRDS(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Predictive Modeling", "Random Model Final Output", "mn_random_leave_out.rds"))
...
predictions_summary <- left_out_data %>% 
...
predictions_summary_original <- left_out_data %>% 
...
lake_species_obs <- left_out_data %>% 
  group_by(dow, spec) %>% 
...
coords <- open_dataset(file.path("G:", 
                                  "Shared drives", 
                                  "Hansen Lab", 
                                  "RESEARCH PROJECTS", 
                                  "Fish Survey Data", 
...
##############################################################################
#now scale with original data
#select variables that we need to scale
scale_vars <- c("log_area",
                "logit_wetlands",
                "logit_urban",
                "logit_ag",
                "log_ws_lake_ratio")

#grab scalers from original data
scalers <- hg.model %>% 
  select(all_of(scale_vars)) %>% 
  summarise(across(everything(),
                   list(mean = ~ mean(.x),
                        sd = ~ sd(.x)))) 
means <- scalers %>%  select(ends_with("_mean")) %>% as.list()
sds <- scalers %>%  select(ends_with("_sd")) %>% as.list()
...
pred_data <- pred_data %>% 
  ...
  mutate(across(all_of(scale_vars),
                ~ (.x - means[[cur_column()]]) / sds[[cur_column()]],
                .names = "scaled_{col}"))
#check if there are differences between scaled values between lakes that exist in both
check <- hg.model %>% 
  distinct(dow, log_area, logit_wetlands, scaled_log_area, scaled_logit_wetlands) %>% 
  left_join(pred_data, by = "dow", suffix = c("_new", "_old")) %>% 
  select(dow, log_area_new, log_area_old, scaled_log_area_new, scaled_log_area_old, logit_wetlands_new, logit_wetlands_old, scaled_logit_wetlands_new, scaled_logit_wetlands_old) %>% 
  mutate(diff_area = scaled_log_area_new - scaled_log_area_old,
         diff_wetlands = scaled_logit_wetlands_new - scaled_logit_wetlands_old)
rm(check, sds, means, scale_vars, scalers)
...
pred_spec <- tibble(spec = c("WAE",
                    ...
                    length = c(15,
                               ...
                               12,
                               8.1))
length_scalers <- hg.model %>% 
  group_by(spec) %>% 
  summarise( length_mean = mean(lgthin),
             length_sd   = sd(lgthin))
...
rm(pred_data, pred_spec, length_scalers)
...
ggplot() +
  geom_sf(data = states, fill = "white", color = "black") +
  geom_sf(data = lake_sf, 
          aes(alpha = .prediction, shape = risk), 
          color = "blue", size = 2) +
  facet_wrap(~ spec) +
  theme_minimal()
```
...
#things to clean up:
-move scaled co variate calculations to after leave out 
    -rerun model
-save those parameters used in that calculation
-compare these to where the DNR has consumption advisories 