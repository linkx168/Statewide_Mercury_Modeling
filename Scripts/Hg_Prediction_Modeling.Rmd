...
res.summary <- read_csv("summary_residuals.csv") %>% 
  ...
  mutate(trans_observed = exp(log_hg),
         trans_predicted = exp(fitted.value),
         squared_errors = (trans_observed - trans_predicted) *2)
...
res.summary %>% 
  group_by(SPEC) %>% 
  summarise(rmse = sqrt(mean(squared_errors)))
...
#fitted values vs. length.. how are we hoping up?
res.summary %>% 
  ggplot() +
