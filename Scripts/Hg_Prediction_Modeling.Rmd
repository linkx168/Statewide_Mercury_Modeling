...
lakes.sum <- lakes %>% 
  ...
  group_by(parent_dow, SPEC, scaled_length) %>% 
...
#initial plot, play around with alpha in the future 
ggplot() +
  stat_smooth(data = lakes.sum, aes(LGTHIN, prob, group = parent_dow, color = SPEC), geom = "smooth", se = F) +
  stat_smooth(data = species.sum, aes(LGTHIN, prob), geom = "smooth", se = F, color = "black") +
...
#playing around with density
ggplot() +
  stat_smooth(data = lakes.sum, aes(LGTHIN, prob, group = parent_dow, color = SPEC), geom = "smooth", se = FALSE) +
  geom_hline(yintercept = 0.5, linetype = "dashed") +
  geom_hex(data = lakes.sum, aes(LGTHIN, prob), bins = 30, alpha = 0.5) +
...
#zebra mussel effect - generate some figures from zm lakes
zm_lakes_pre_post <- hg.data %>% 
