...
res.summary <- res %>%
  median_qi() 
write_csv(res.summary, "summary_residuals.csv")
...
#predicted probability of seeing an observation less than or equal to the actual observation 
res.prob <- hg.data %>% 
  filter(!(DOW %in% c("16063300", 
                      "31122500",
                      "38021100",
  ...
  select(DOW,
         ...
         log_hg) %>% 
  add_predicted_draws(fit.statewide) %>% 
  summarise(p_residual = mean(.prediction < log_hg), #posterior predictive p-value
            z_residual = qnorm(p_residual), #apply inverse cumulative distribution function to get quantile residuals
            .groups = "drop_last")
