...
#Data
Read in mercury and covariate data from the Google drive. The entire contaminate database is read in and filtered accordingly. DNR lakeshed, glm lake meta data, infested waters list, and lake link are used to get covraties for modeling. this results fish mercury observations with attached covariate data 
```{r}
#mercury data
hg <- read_csv(file.path("G:", "Shared drives", "Hansen Lab", "RESEARCH PROJECTS", "Statewide mercury modeling", "Data", "Mercury Data", "allfish_data_04042024_JLO.csv")) %>% 
  #remove any fish without lenght, mercury, or lake data
...
1. further filter and scale data
2. fit model of constant lscale 10 to entire dataset
3. generate a subset of lake-spec combos to be left out 
4. fit model of constant lscale 10 with 15% of spec-lake combos left out
5. generates summaries of well the model predicted left out samples at 
        1.) individual level 
        2.) places where one species was left out but another remained 
6. fit model with random effect with loo data
7. generates summaries of prediction of random effects model 
```{r}
...
fit.gp.constant <- brm(log_hg ~ 
              ...
              prior = prior(constant(10), class = "lscale", coef = "gpeastingnorthing"),
                     ...
                     family = gaussian(),
                     cores = 12)
#saveRDS(fit.gp.constant, "fit.gp.constant.rds")
fit.gp.constant <- readRDS("fit.gp.constant.rds")

#quick look at the summary
summary(fit.gp.constant)
plot(fit.gp.constant)

#############leave out analysis########################## 
#here is the saved data from the extraction - or you can run the code below
hg.data.loo <- read_csv("loo_data_constant_lscale.csv")
################################################################
...
#this shows that 13 lakes were completely removed 
#17000300 had two species removed 

hg.data %>%
  distinct(DOW, SPEC) %>%
  anti_join(hg.data.loo, by = c("SPEC", "DOW")) %>% 
  semi_join(hg.data.loo, by = "DOW") 
#this shows that there are 457 cases where one spec was removed from a lake at least one species remains


#saving loo data
#write_csv(hg.data.loo, "loo_data_constant_lscale.csv")

#modeling
fit.gp.loo <- brm(log_hg ~ 
                scaled_length*SPEC + 
                SPEC + 
                scaled_logit_wetlands + 
                scaled_logit_urban +
              ...
              prior = prior(constant(10), class = "lscale", coef = "gpeastingnorthing"),
                     ...
                     family = gaussian(),
                     cores = 12)
#saveRDS(fit.gp.loo, "statewide_gp_constant_loo.rds")
fit.gp.loo <- readRDS("statewide_gp_constant_loo.rds")
...
loo.data <- hg.data %>% 
  anti_join(hg.data.loo)

# build predictions from the left out data - read in saved data to save time
# predictions <- loo.data %>% 
#   add_predicted_draws(fit.gp.loo, ndraws = 100)
# write_csv(predictions, "predictions_loo_constant_lscale.csv")
predictions <- read_csv("predictions_loo_constant_lscale.csv")

#summary of the draws for each prediction
...
predictions_summary <- loo.data %>%
  #this is nessary due to there being some duplicate spec-length-hgppm-year-lake
...
predictions_summary %>% 
  ggplot() +
  geom_point(aes(log_hg, .prediction, color = lake_loo)) +
...
predictions_summary_original <- loo.data %>%
  #this is nessary due to there being some duplicate spec-length-hgppm-year-lake
  distinct(DOW, SPEC, HGPPM, LGTHIN,  .keep_all = T) %>%
...
#####comparing left out species####
#generate a list of species that remained in a lake while at least one was removed
# Identify species removed
removed_species <- hg.data %>%
  distinct(DOW, SPEC) %>%
  anti_join(hg.data.loo, by = c("SPEC", "DOW"))

# Identify species kept
kept_species <- hg.data.loo %>%
  distinct(DOW, SPEC)

# Merge to see which species were removed and which were kept in the same lake
species_changes <- removed_species %>%
  semi_join(kept_species, by = "DOW") %>%
  full_join(kept_species, by = "DOW", suffix = c("_removed", "_kept")) %>%
  group_by(DOW) %>%
  summarise(
    removed_species = list(unique(SPEC_removed[!is.na(SPEC_removed)])),
    kept_species = list(unique(SPEC_kept[!is.na(SPEC_kept)]))
  ) %>%
  mutate(
    removed_species = sapply(removed_species, function(x) ifelse(length(x) == 0, NA, paste(x, collapse = ", "))),
    kept_species = sapply(kept_species, function(x) ifelse(length(x) == 0, NA, paste(x, collapse = ", ")))
  )
species_changes
rm(removed_species, kept_species)

species_changes_remain <- species_changes %>% 
  filter(!is.na(kept_species), !is.na(removed_species))

sort_species <- function(species) {
  species %>% 
    str_split(", ") %>% 
    map_chr(~ paste(sort(.x), collapse = ", "))
}

# Apply function to standardize species pairs
species_changes_remain <- species_changes_remain %>%
  mutate(kept_species = sort_species(kept_species),
         removed_species = sort_species(removed_species)) %>% 
  mutate(pred_fish_removed_no_remain = case_when((str_detect(removed_species, "NOP") |
                                                   str_detect(removed_species, "WAE")) &
                                                   !(str_detect(kept_species, "NOP") |
                                                   str_detect(kept_species, "WAE")) ~ "y",
                                                 TRUE ~ "n"),
         pred_fish_removed_one_remains = case_when((str_detect(removed_species, "NOP") |
                                                   str_detect(removed_species, "WAE")) &
                                                   (str_detect(kept_species, "NOP") |
                                                   str_detect(kept_species, "WAE")) ~ "y",
                                                 TRUE ~ "n"),
...
species_changes_remain %>% 
  group_by(kept_species) %>% 
  count() %>% 
  arrange(desc(n)) %>% 
  print(n = nrow(.))

species_changes_remain %>% 
  group_by(removed_species) %>% 
  count() %>% 
  arrange(desc(n)) %>% 
  print(n = nrow(.))

species_changes_remain %>% 
  summarise(n_pred_removed_no_remain = sum(pred_fish_removed_no_remain == "y"),
            n_pred_removed_one_remains = sum(pred_fish_removed_one_remains == "y"),
            total = n())


###############random effects loo########################################
fit.ran.loo <- brm(log_hg ~ 
                scaled_length*SPEC + 
                SPEC + 
                scaled_logit_wetlands + 
                scaled_logit_urban +
                ...
                (1|DOW),
                     ...
                     chains = 3,
                     family = gaussian())
#saveRDS(fit.ran.loo, "statewide_random_loo.rds")
fit.ran.loo <- readRDS("statewide_random_loo.rds")
...
# build predictions from the left out data - read in saved data to save time
 predictions <- loo.data %>% 
  add_predicted_draws(fit.ran.loo, ndraws = 100, allow_new_levels = T)
#write_csv(predictions, "predictions_loo_rand.csv")
predictions <- read_csv("predictions_loo_rand.csv")
...
# differnces between random and gp in prediction
```{r}
#mercury data ready for modeling after filtering on covaraite data
total_data <- read_csv(file.path("D:",
                     "Shared drives",
                     "Hansen Lab",
                     "RESEARCH PROJECTS",
...
#data with 15% of lake-spec combos left out
full_loo_data <- read_csv(file.path("D:",
                     "Shared drives",
                     "Hansen Lab",
                     "RESEARCH PROJECTS",
                     "Statewide mercury modeling",
                     "Predictive Modeling",
                     "Model Outputs",
                     "loo_data_constant_lscale.csv"))

#left out data
left_out_data <- total_data %>% 
  anti_join(full_loo_data)

#gp model fit with constant lscale at 10 using loo data
gp_fit <- readRDS(file.path("G:",
                     "Shared drives",
                     "Hansen Lab",
                     "RESEARCH PROJECTS",
                     "Statewide mercury modeling",
                     "Predictive Modeling",
                     "Model Outputs",
                     "statewide_gp_constant_loo.rds"))

#random effects model fit using loo data
rand_fit <- readRDS(file.path("G:",
                     "Shared drives",
                     "Hansen Lab",
                     "RESEARCH PROJECTS",
                     "Statewide mercury modeling",
                     "Predictive Modeling",
                     "Model Outputs",
                     "statewide_random_loo.rds"))
###############################################################################
# compare betas
gp_coefs <- fixef(gp_fit)
rand_coefs <- fixef(rand_fit)

head(gp_coefs)
head(rand_coefs)

gp_coefs <- as.data.frame(gp_coefs) %>% 
  rownames_to_column("term") %>% 
  mutate(model = "gp")

rand_coefs <- as.data.frame(rand_coefs) %>% 
  rownames_to_column("term") %>% 
  mutate(model = "random")
coef_compare <- bind_rows(gp_coefs, rand_coefs)
rm(gp_coefs, rand_coefs)
##############################################################################
coef_wide <- coef_compare %>%
  select(term, model, Estimate) %>%
  pivot_wider(names_from = model, values_from = Estimate) %>%
  mutate(diff = gp - random)

ggplot(coef_wide %>% 
         mutate(type = case_when(str_detect(term, "SPEC") ~ "species",
                                 TRUE ~ "lake")), aes(x = random, y = gp, label = term)) +
  ...
  labs(
    x = "Random effects estimates",
    y = "Gaussian Process estimates"
  ) +
  theme(legend.position = "bottom")
ggsave("beta_compare.jpg", height = 5, width = 7, dpi = 600)
rm(coef_compare, coef_wide)
###########################################################################
gp_preds   <- posterior_predict(gp_fit, newdata = left_out_data, ndraws = 300)
rand_preds <- posterior_predict(rand_fit, newdata = left_out_data, ndraws = 300, allow_new_levels = T)

summarize_preds <- function(pred_matrix, observed) {
  apply(pred_matrix, 2, function(x) {
    c(
      mean   = mean(x),
      median = median(x),
      sd     = sd(x),
...
gp_summary <- summarize_preds(gp_preds, left_out_data$log_hg)
rand_summary <- summarize_preds(rand_preds, left_out_data$log_hg)

gp_summary$model <- "gp"
rand_summary$model <- "random"

pred_summary <- bind_rows(gp_summary, rand_summary)


ggplot(pred_summary, aes(x = obs, y = median, color = model)) +
  geom_point(alpha = 0.5) +
  geom_errorbar(aes(ymin = `l95.2.5%`, ymax = `u95.97.5%`), alpha = 0.2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  labs(x = "Observed log(Hg)",
       y = "Predicted median log(Hg)") +
  theme_minimal() +
  facet_wrap(~model)
#ggsave("gp_random_obs_vs_fitted.jpg", height = 5, width = 6, dpi = 600)
rm(pred_summary, gp_preds, rand_preds)

compute_metrics <- function(df) {
  data.frame(
    MAD  = median(abs(df$median - df$obs)),  # median absolute deviation
    RMSE = sqrt(mean((df$mean - df$obs)^2)), # root mean squared error
    CI_width = mean(df$u95 - df$l95),        # avg 95% CI width
    avg_sd   = mean(df$sd),                  # avg posterior sd
    coverage = mean(df$obs >= df$l95 & df$obs <= df$u95) * 100 # % within 95% CI
  )
}

gp_metrics   <- compute_metrics(gp_summary)   %>% mutate(model = "gp")
rand_metrics <- compute_metrics(rand_summary) %>% mutate(model = "random")

metrics_all <- bind_rows(gp_metrics, rand_metrics)
rm(gp_metrics, rand_metrics, metrics_all)
#############################################################################
train_lakes <- unique(full_loo_data$DOW)
left_out_data <- left_out_data %>% 
  mutate(lake_status = case_when(DOW %in% train_lakes ~ "partial",
                                 TRUE ~ "new"))

train_species_by_lake <- full_loo_data %>% 
  group_by(DOW) %>% 
...
left_out_data %>%
  ...
  count()
#13 new lakes, 43 that do no have wae or nop, and 322 that have one or the other 

gp_summary <- summarize_preds(posterior_predict(gp_fit, 
                                                newdata = left_out_data, 
                                                ndraws = 300),
                              left_out_data$log_hg
                              ) %>% 
  bind_cols(left_out_data %>% 
              select(DOW, SPEC, species_case))

rand_summary <- summarize_preds(posterior_predict(rand_fit, 
                                                  newdata = left_out_data,
                                                  ndraws = 300,
                                                  allow_new_levels = T),
                                left_out_data$log_hg) %>% 
...
gp_metrics <- gp_summary %>%
  group_by(species_case) %>%
  summarise(
    MAD  = median(abs(median - obs)),
    RMSE = sqrt(mean((mean - obs)^2)),
    CI_width = mean(`u95.97.5%` - `l95.2.5%`),
    avg_sd   = mean(sd),
    coverage = mean(obs >= `l95.2.5%` & obs <= `u95.97.5%`) * 100,
    .groups = "drop"
  ) %>%
  mutate(model = "gp")

rand_metrics <- rand_summary %>%
  group_by(species_case) %>%
  summarise(
    MAD  = median(abs(median - obs)),
    RMSE = sqrt(mean((mean - obs)^2)),
...
metrics_long <- metrics_all %>%
  pivot_longer(
    cols = c(MAD, RMSE, CI_width, avg_sd, coverage),
    names_to = "metric",
    values_to = "value"
  )
metrics_long %>% 
...
ggplot(metrics_long, aes(x = model, y = value, fill = model)) +
  ...
  facet_grid(metric ~ species_case, scales = "free_y") +
  labs(x = "Model type", y = "Metric value") +
  theme_minimal() +
  theme(legend.position = "none") 
#ggsave("metrics_levels.jpg", height = 6, width = 6, dpi = 600)

# Filter only the new lake cases
new_lake_preds <- bind_rows(
  gp_summary %>% mutate(model = "gp"),
  rand_summary %>% mutate(model = "random")
) %>%
  filter(species_case == "new_lake")

# Plot observed vs predicted
ggplot(new_lake_preds, aes(x = obs, y = median, color = model)) +
  ...
  labs(
    x = "Observed log(Hg)",
    y = "Predicted log(Hg)",
    title = "Observed vs Predicted for New Lakes",
    subtitle = "Comparing Gaussian Process vs Random effects"
  ) +
  theme_minimal()
#ggsave("new_lake_predictions.jpg", height = 5, width = 5, dpi = 600)

new_spec_with_wae_nop <- bind_rows(
  gp_summary %>% mutate(model = "gp"),
  rand_summary %>% mutate(model = "random")
) %>%
...
# Plot observed vs predicted
ggplot(new_spec_with_wae_nop, aes(x = obs, y = median, color = model)) +
  geom_point(alpha = 0.7, size = 2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed") +
  facet_wrap(~model) +
  labs(
    x = "Observed log(Hg)",
    y = "Predicted log(Hg)",
    title = "Observed vs Predicted for species (wae or nop present)",
    subtitle = "Comparing Gaussian Process vs Random effects"
  ) +
  theme_minimal()
#ggsave("new_spec_with_nop_wae.jpg", height = 5, width = 5, dpi = 600)
```
