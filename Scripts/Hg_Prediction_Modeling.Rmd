...
#filtering for wae/nop in recent years
hg.data <- hg.model %>% 
...
fit.statewide <- brm(log_hg ~ scaled_length + SPEC + scaled_logit_wetlands + scaled_WS_Lake_Ratio + log_area + scaled_clarity + (1|DOW),
                     data = hg.data,
                     iter = 50000,
                     warmup = 10000,
                     chains = 3,
...
#exploring residuals - not running and will have to explore later
qres <- hg.data %>%
...
#posterior preditive check - might want to do this at various levels (species)?
pp_check(fit.statewide, nsamples = 500) 
```
