...
# Building Leave Out Test 
```{r}
#filter data based on basics 
hg.data <- hg.model %>% 
...
#how many lakes have wae and nop?
hg.data %>% 
  group_by(DOW) %>% 
  mutate(has_wae_nop = case_when(SPEC == "WAE" ))
...
#find the sd of the draws for each prediction
predictive_sd <- predictions %>% 
  group_by(.row) %>% 
  summarise(.pred_sd = sd(.prediction),
            median = median(.prediction))

predictions_summary <- predictions_summary %>% 
  left_join(predictive_sd)
rm(predictive_sd)
...
predictive_sd_original <- predictions %>% 
  mutate(.prediction = exp(.prediction)) %>%
  group_by(.row) %>% 
  summarise(.pred_sd = sd(.prediction),
            median = median(.prediction))
...
predictions_summary_original <- predictions_summary_original %>% 
  left_join(predictive_sd_original)
rm(predictive_sd_original)
